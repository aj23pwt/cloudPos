...
BEGIN
   ...
   DECLARE CONTINUE HANDLER FOR SQLEXCEPTION SET t_error = 1;
	...
	SET arg_ret = 1 ;
     ...
     IF arg_hotel_id IN ('10191','10190') THEN
...
IF(i=0) THEN
		 ...
		 UPDATE pos_master a ,      
		 ( SELECT hotel_group_id,hotel_id,accnt ,SUM(amount1) AS amount1 ,SUM(amount2) AS amount2 ,SUM(amount3) AS amount3  
																								 FROM  pos_detail WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id 
																								 GROUP BY hotel_group_id,hotel_id,accnt )  b  
     SET a.amount1 = b.amount1,a.amount2 = b.amount2 ,a.amount3 = b.amount3 
     WHERE a.hotel_group_id = b.hotel_group_id AND a.hotel_id = b.hotel_id AND a.accnt = b.accnt 
...
IF NOT EXISTS ( SELECT 1 FROM  pos_pccode_audit_flag  WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id  AND is_audit = 'T' AND is_rmposted <> 'T' AND biz_date <= var_bizdate) THEN
		...
		ELSE
		     ...
		     ELSE
		        ...
		        START TRANSACTION;
				       ...
				       delete a.* from pos_dish a,pos_menu b,pos_interface_map c,pos_pccode d where a.hotel_group_id = arg_hotel_group_id and a.hotel_id = arg_hotel_id and a.biz_date = arg_biz_date and b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id and b.biz_date = arg_biz_date
							AND b.menu_no LIKE 'P%' AND c.hotel_group_id = arg_hotel_group_id AND c.hotel_id = arg_hotel_id AND c.link_type = 'ta_code' and d.hotel_group_id = arg_hotel_group_id AND d.hotel_id = arg_hotel_id and a.menu_id = b.id and b.pos_code = c.code and c.pos_code = d.code ;

				       delete a.* from pos_menu a,pos_interface_map b,pos_pccode c where a.hotel_group_id = arg_hotel_group_id and a.hotel_id = arg_hotel_id and a.biz_date = arg_biz_date and b.hotel_group_id = arg_hotel_group_id and b.hotel_id = arg_hotel_id and b.link_type = 'ta_code'
							AND a.menu_no LIKE 'P%' and c.hotel_group_id = arg_hotel_group_id and c.hotel_id = arg_hotel_id and a.pos_code = b.code and b.pos_code = c.code;	                   
		--                  DELETE FROM pos_menu WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bizdate ;
											INSERT  INTO pos_menu (`hotel_group_id` , `hotel_id` , `biz_date` ,`pos_station` ,`pos_code`,`pos_descript`,`menu_no`, `table_no`,
												`cashier`,`guest_num`,`pay_code`,`pay_descript`,`rmno`,`guest_id`,`guest_name`,`company_id`,`company_name`,
												`ar_account`,`card_code`,`card_no`,`discount_mode`,`fee`,`fee_srv`,`fee_dsc`,`sales`,`sales_man`,`last_ls_no`,
		...
		--                DELETE FROM pos_dish WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bizdate ;
		                ...
		                INSERT INTO pos_dish (`hotel_group_id`,`hotel_id`,`menu_id`,`biz_date`,`pos_station` ,`code`,`descript`,`fee`, `list_order`,`create_user`,`create_datetime`,`modify_user`,`modify_datetime`,`cashier`,`inumber`,`remark`)               			
					SELECT f.hotel_group_id,f.hotel_id,0,f.biz_date,c.ta_code,a.code,a.descript,f.amount-f.amount1,'0',f.userCode,f.create_datetime,f.userCode,f.modify_datetime,f.shift,'1',f.accnt
						FROM pos_audit_master f lEFT JOIN pos_interface_map a ON a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id
						...
						WHERE f.hotel_group_id = arg_hotel_group_id AND f.hotel_id = arg_hotel_id AND f.code = b.code AND f.pccode = c.code AND f.biz_date = var_bizdate AND f.type = '1' AND a.pos_code = b.flag AND a.sys_code=c.code
						ORDER BY f.accnt;
   ...
   SET  arg_msg = "1,系统初始化完成!" ;  
        ...
        START TRANSACTION;
                  ...
                  -- DELETE FROM  pos_mode_def where hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id  ;
               	 DELETE FROM  pos_audit_master WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id  ;
               	 DELETE FROM  pos_audit_report WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id  ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `up_pos_rep_daily_month`(
	IN arg_hotel_group_id	BIGINT(16),
	IN arg_hotel_id 	BIGINT(16),
	IN arg_biz_date		DATETIME,
	IN arg_tag		CHAR(1)
...
BEGIN
	...
	DECLARE var_amount_d_total 	DECIMAL(10,2);
	
	DROP TEMPORARY TABLE IF EXISTS tmp_pos_master;
	DROP TEMPORARY TABLE IF EXISTS tmp_pos_account;
	DROP TEMPORARY TABLE IF EXISTS tmp_pos_detail;
	DROP TEMPORARY TABLE IF EXISTS tmp_pos_bill_pre;
	DROP TEMPORARY TABLE IF EXISTS tmp_pos_bill;
	...
	CREATE TEMPORARY TABLE tmp_pos_bill(
		id BIGINT(16) NOT NULL AUTO_INCREMENT,
		tag 		CHAR(1),
		plu_code	VARCHAR(20),
		plu_des 	VARCHAR(100),
 	...
 	INSERT tmp_pos_bill(tag,plu_code,plu_des,number,price,unit,info1,info2,amount,amount_d)
	...
	SELECT SUM(amount_d) INTO var_amount_d_total FROM tmp_pos_bill WHERE tag = 'A';
	INSERT tmp_pos_bill(tag,plu_code,plu_des,number,price,unit,info1,info2,amount,amount_d)
		SELECT 'D','D01','合计','','','','','','',SUM(var_amount_d_total) ;
		
	UPDATE tmp_pos_bill SET amount = CAST(amount_d AS CHAR) WHERE tag = 'D';
	INSERT tmp_pos_bill(tag,plu_code,plu_des,number,price,unit,info1,info2,amount,amount_d)
		SELECT 'E','','','','','','','','',0;
	INSERT tmp_pos_bill(tag,plu_code,plu_des,number,price,unit,info1,info2,amount,amount_d)
		SELECT 'E','','','','','','','','',0;
	SELECT amount_d INTO var_amount_total FROM tmp_pos_bill WHERE tag = 'D';
	INSERT tmp_pos_bill(tag,plu_code,plu_des,number,price,unit,info1,info2,amount,amount_d)
		SELECT 'F','F01',CONCAT('合计:',TO_UPPER_NUM(var_amount_total,2)),'','','','','',CAST(var_amount_total AS CHAR),var_amount_total;
 	-- INSERT tmp_pos_bill(tag,info1,info2,amount)
 	INSERT tmp_pos_bill(tag,plu_code,plu_des,number,price,unit,info1,info2,amount,amount_d)
	...
	SELECT tag,plu_code,plu_des,number,price,unit,info1,info2,amount,amount_d FROM tmp_pos_bill ORDER BY id;	
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `up_pos_audit_not_balance_check`(
	IN arg_hotel_group_id	BIGINT(16),	
	IN arg_hotel_id		BIGINT(16),
	IN arg_biz_date		DATETIME
	)
...
DROP PROCEDURE IF EXISTS `up_pos_table_map_yh`;
DELIMITER ;;
CREATE  PROCEDURE `up_pos_table_map_yh`(IN arg_hotel_group_id	BIGINT(16),	
	IN arg_hotel_id		BIGINT(16),
	IN arg_date		DATETIME,
	IN arg_shift 		VARCHAR(1),
	IN arg_pccode 		VARCHAR(256),
	...
	-- IF (time_diff>5) THEN
         ...
         IF(arg_flag='0' OR arg_flag='1') THEN
				-- 已开单pos_account为准 信息插入 预订信息插叙
					UPDATE tmp_pos_table_map a,( SELECT b.hotel_group_id,b.hotel_id,b.accnt,c.gsts,c.pcrec,c.empid,c.phone,c.shift,b.biz_date,b.tableno,c.extra FROM pos_account b 
																	     LEFT JOIN  pos_master c  ON  b.hotel_group_id = c.hotel_group_id AND b.hotel_id = c.hotel_id AND b.accnt = c.accnt
																	     WHERE b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.number = '1' AND b.sta = 'I' AND c.type1 NOT IN ('WM','KC') AND c.type2 <>'FD') d 
            ...
            -- 已开单pos_account为准 信息插入 台位资源查询
					UPDATE tmp_pos_table_map a,( SELECT b.hotel_group_id,b.hotel_id,b.accnt,c.gsts,c.pcrec,c.empid,c.phone,c.shift,b.biz_date,b.tableno,c.extra FROM pos_account b 
																	     LEFT JOIN  pos_master c  ON  b.hotel_group_id = c.hotel_group_id AND b.hotel_id = c.hotel_id AND b.accnt = c.accnt
																	     WHERE b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.number = '1' AND b.sta = 'I' AND c.type1 NOT IN ('WM','KC') ) d 
					  SET a.accnt = d.accnt ,a.sta='2',a.gsts=d.gsts,a.logdate=NOW(),a.pcrec=d.pcrec,a.empid=d.empid,a.base='I',a.phone=d.phone, a.shift = d.shift , a.masterExtra = d.extra
						WHERE  a.hotel_group_id = d.hotel_group_id AND a.hotel_id = d.hotel_id  AND a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id
									 AND  (TO_DAYS(d.biz_date)-TO_DAYS(arg_date) = 0 ) 
								    -- AND  (d.shift = arg_shift OR arg_shift = '')
        ...
        INSERT tmp_pos_table_map (hotel_group_id,hotel_id,pccode,tableno,accnt,inumber,gsts,sta,logdate,dishs,checks,pcrec,amount,empid,tblchk,base,selected,unitname,phone,shift,arrtime,tblname,masterExtra)
				SELECT  arg_hotel_group_id,arg_hotel_id,a.pccode,a.tableno,a.accnt,'1',c.gsts,'2',a.biz_date,0,0,c.pcrec,0,a.empid,'','I','','','',a.shift,'',a.tableno,c.extra
				FROM pos_account a -- pos_pccode_table b,
				LEFT JOIN  pos_master c  ON  a.hotel_group_id = c.hotel_group_id AND a.hotel_id = c.hotel_id AND a.accnt = c.accnt
				WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.number = '1' AND c.type1 NOT IN ('WM','KC') AND c.type2 = 'FD'
								  -- AND INSTR(b.code,a.tableno) > 0 
								  -- AND (INSTR(CONCAT(',',arg_pccode,','),CONCAT(',',a.pccode,',')) > 0 OR arg_pccode = 'all')
							      -- (a.pccode IN ( SELECT pos_value FROM tmp_pos_splittable ) OR arg_pccode = 'all') 
								  AND a.sta = 'I' 
