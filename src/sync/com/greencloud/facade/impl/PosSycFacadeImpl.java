	...
	*/
	public Boolean saveSyncMasterByAccnt(PosAccntSync posAccntSync){		
		...
		for (int i = 0; i < SycMasterEntityArr.length; i++) {
			...
			}else if("PosDetail".equalsIgnoreCase(SycMasterEntityArr[i]) && T.size() ==0){
				Map<String, String> upParams = new HashMap<String, String>();
				upParams.put("entity", SycMasterEntityArr[i]);
				upParams.put("hotelGroupId", posAccntSync.getHotelGroupId().toString());
				upParams.put("hotelId", posAccntSync.getHotelId().toString());
				upParams.put("accnt", posAccntSync.getAccnt());
				String upUrl = sycUrl
						+ "/PosSycRemoteUpload/updatePosDetailByAccnt.do";
				try {
					response = hp.sendPost(upUrl, upParams, propertys,
								"UTF-8").getContent();
					j = JSON.parseObject(response);
...
//		24* 3600000
		if(serverCode.indexOf("ClOUD") == 0){
			Date d1;
			SysOption sysOption =  sysOptionService.findSysOptionByCatalogItem("system", "pos_biz_date",user.getHotelGroup().getId(),user.getHotel().getId());
			if(sysOption != null){
				try {
					SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");
					d1 = sdf.parse(sysOption.getSetValue());
					long betweent = d1.getTime() - user.getBizDate().getTime();
					if(betweent != 24* 3600000){
						d1.setTime(d1.getTime() - 24* 3600000);
						user.setBizDate(d1);
					}				
		...
		url = url+ "/PosSycRemoteUpload/updatePosServerRegisterAuditFlag.do";
				...
				JSONObject	j = JSON.parseObject(response);	
				if(!"true".equalsIgnoreCase(j.getString("success"))){
					try{
						response = hp.sendPostJson(url,"POST",JSON.toJSONString(user),params, propertys, "UTF-8").getContent();
					} catch (IOException e) {
						e.printStackTrace();
		 ...
		 if("F".equalsIgnoreCase(doType) && "true".equalsIgnoreCase(j.getString("success"))){
			 int ret = posSubService.selectCountPosAuditDate(user.getHotelGroup().getId(),user.getHotel().getId(),user.getBizDate());
			 if(ret == 1){
				 posSubService.updatePosAuditDateEndDate(user.getHotelGroup().getId(),user.getHotel().getId(),user.getBizDate());
			 }else{
				 SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");				 
				 posSubService.updatePosAuditDate(user.getHotelGroup().getId(),user.getHotel().getId(), sdf.format(UserManager.getBizDate()), UserManager.getBizDate(), UserManager.getUserCode(), UserManager.getWorkStationCode());
			 }
