  ...
  private void spellSQL(SQLHelper sh,PosPluAll posPluAll)
  {
    if(posPluAll != null){
       ...
       if(posPluAll.getPtNum()!= null){
        ...
        if(isNotNull(posPluAll.getScanCode())){
            sh.appendSql(" AND obj.scanCode = ? ");
            sh.insertValue(posPluAll.getScanCode().trim());
         }
        if(isNotNull(posPluAll.getUidCode())){
            sh.appendSql(" AND obj.uidCode = ? ");
            sh.insertValue(posPluAll.getUidCode().trim());
         }
        if(isNotNull(posPluAll.getSubjectScanCode())){
            sh.appendSql(" AND obj.subjectScanCode = ? ");
            sh.insertValue(posPluAll.getSubjectScanCode().trim());
         }
        if(isNotNull(posPluAll.getSubject())){
            sh.appendSql(" AND obj.subject = ? ");
            sh.insertValue(posPluAll.getSubject().trim());
         }
