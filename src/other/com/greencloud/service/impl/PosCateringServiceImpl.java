...
*/		
		...
		+ " FROM pos_res a LEFT JOIN pos_pccode b ON a.hotel_group_id=b.hotel_group_id AND a.hotel_id=b.hotel_id AND a.pccode = b.code  WHERE 1=1 ");
		 ...
		 List<Catering> cateirnglist = posSubDao.getDataBySql(Catering.class,sh);
		 //加入开单已经开台的 主单信息
		 SQLHelper sh1 = new SQLHelper(" SELECT a.hotel_group_id as hotelGroupId,a.hotel_id as hotelId, a.id as rsvId ,a.accnt as catNo,'已开单' as rsvName,'M' as rsvType,a.sta, a.biz_date as dateBegin, a.biz_date as dateEnd,a.tableno as cateringPlace, '' as theme , a.info1 as remark ,a.shift as category , a.pccode as extraFlag ,b.descript as board  "
					+ " FROM pos_account a LEFT JOIN pos_pccode b ON a.hotel_group_id=b.hotel_group_id AND a.hotel_id=b.hotel_id AND a.pccode = b.code  WHERE 1=1 ");
					 sh1.appendSql(" AND a.hotel_group_id = ? ");
					 sh1.insertValue(hotelGroupId);
					 sh1.appendSql(" AND a.hotel_id = ? ");
					 sh1.insertValue(hotelId);
					 sh1.appendSql(" AND a.biz_date >= ? ");
					 sh1.insertValue(dateBegin);
					 sh1.appendSql(" AND a.biz_date <= ? ");
					 sh1.insertValue(dateEnd);
					 sh1.appendSql(" AND a.number = '1' AND sta = 'I' ");
		 List<Catering> cateirnglist1 = posSubDao.getDataBySql(Catering.class,sh1);
		if(cateirnglist1 !=null && cateirnglist1.size()>0){
			cateirnglist.addAll(cateirnglist1);
		}			 
		//遍历每个桌号的订单区间信息
