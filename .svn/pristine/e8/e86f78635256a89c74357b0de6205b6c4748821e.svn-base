...
public String updateTableNoByAccnt(final long hotelGroupId, final long hotelId,
	...
	@Override
	public String updateKaiyuanVipDiscount(final long hotelGroupId,final long hotelId,final Date bizDate,final String accnt,final String cardno,final String mode,final BigDecimal dsc,final BigDecimal srv,final BigDecimal tax) {
		String sql = "{Call up_pos_detail_dsc_for_ky(?,?,?,?,?,?,?,?,?,?)}";	
		final java.sql.Date sqlDate = new java.sql.Date(bizDate.getTime());
		@SuppressWarnings({ "unchecked", "rawtypes" })
		Object object = getJdbcTemplate().execute(sql,new CallableStatementCallback(){
			public Object doInCallableStatement(CallableStatement cs) throws SQLException, DataAccessException 
			{		
			     ...
			     cs.setDate(3, sqlDate);
			     cs.setString(4,accnt);
			     cs.setString(5,cardno);
			     cs.setString(6, mode);
			     cs.setBigDecimal(7, dsc);
			     cs.setBigDecimal(8, srv);
			     cs.setBigDecimal(9, tax);
			     cs.registerOutParameter(10,java.sql.Types.VARCHAR);		     
			     cs.execute();
			     return cs.getString(10);
			}
